...
public class GetUser{
	...
	//public static List<String> al_fails;
			...
			if( !dm.mdata.get("id").equals("") ){
				...
				for( int i=1;i<dm.sattr.length;i++ ){
					if( dm.mdata.get(dm.sattr[i]) != null && !dm.mdata.get(dm.sattr[i]).equals("") ){//null becomes "" ?
						...
						//query = "update users set " + dm.sattr[i] + "=" +  "\'"+dm.mdata.get(dm.sattr[i]).replace("\\","\\\\").replace("'","\\'")+"\'" + " where id =" + dm.mdata.get("id");
						//dm.statement.execute(query);
					}
				}
				headers += ") ";
		...
		while(al_ids.size()>0){
			...
			for(int i=0;i<al_tokens.size();i++){			
				try{		
					...
					/*
					System.out.println("********************************************************");
					System.out.println("Now the "+ String.valueOf(i+1)+ "th token:");
					System.out.println("the remaining user_timeline is:" + String.valueOf(user_timeline_remaining) );
					System.out.println("the remaining /users/show/:id is:" + String.valueOf(users_show_remaining) );
					System.out.println("now the set has:"+limitReachingTokenIndeces.size() + "toekens");
					*/			
					//Thread.sleep(2500);
                    ...
                    // e.printStackTrace();//
                    // System.out.println("Line Exception: " +e.getStackTrace()[0].getLineNumber());
					System.err.println("Token " + i+" - start function error in GetUser:"+ e.getMessage());

					continue;
				}
			}
			
			/*if( !work ){not_work++;}else{not_work=0;}
			if(not_work == 2) {
				not_work=0;
				System.out.println("Limit reached! sleep one hour!!\nSuspended at " + (new Date()));
