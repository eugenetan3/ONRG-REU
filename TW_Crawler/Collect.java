/**
 * Unify the four parts of code. pay attention to the buildTable part.(`users`, `tweets`,
 * and `urls` table names are included manually!!! fr_fo are not included
 *
 * @author morgan
...
//      //if al_tables is not input, then do is automatically based on time
	...
	public static void main(String[] args) throws Exception{
		...
		// System.out.println("\t\t"+al_ids.size() + " :: " + al_tokens.size());

		/* test of the db_info.txt file
		System.out.println(host_str+" "+db_name+" "+username+" "+password+" "+port_str);
		for(String s:al_tables){
		...
		if( collect_userinfo ){
			...
			// System.out.println("2\t\t"+al_ids.size() + " :: " + al_tokens.size());
