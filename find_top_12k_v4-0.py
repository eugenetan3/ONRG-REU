        ...
        if os.path.isfile(path + '/' + f) and f != 'all' and len(f.split('-')) > 2:
            ...
            with open(path + '/' + f) as fp:
                for line in fp:
                    ...
                    int(new_line[0]), new_line[1], new_line[4], new_line[5])
...
#chunk of code that uploads social information of the other_significants into table
    ...
    query = "SELECT friend_id, count(distinct follower_id) FROM Elite_Weekly.fofr_%s group by friend_id INTO OUTFILE '/research/mysql_output/friends1_%s' fields terminated by ',' lines terminated by '\n';" % (date,date)
    cursor = dbm.cursor()
    cursor.execute(query)
    
  
    command = 'cp /research/mysql_output/friends1_%s /home/eugenet/mysql_output/friends_%s' % (date, date) 
    ...
    social_query = "LOAD DATA LOCAL INFILE '/home/eugenet/mysql_output/most_followed_friends_social_%s' INTO TABLE Elite_Weekly.social_baker FIELDS TERMINATED BY ',' LINES TERMINATED BY '\n';" % (
    date)
    cursor = dbm.cursor()
    #cursor.execute(social_query)
    os.rename('/home/eugenet/mysql_output/most_followed_friends_social_' + date,
              base_path + 'most_followed_friends_social')

    print ("create a table that contains initial 12000 users.")
    os.rename(base_path + 'top12k', '/home/eugenet/mysql_output/top12k_' + date)
    ...
    q = "SELECT distinct(id) FROM Elite_Weekly.users INTO OUTFILE '/research/mysql_output/top12k-%s' LINES TERMINATED BY '\n';" % (date)
    cursor.execute(q.encode('utf-8'))
    command = 'cp /research/mysql_output/top12k-%s /home/eugenet/mysql_output/' % date
    os.system(command)


    id_file = '/home/eugenet/mysql_output/top12k-%s' % date
    token_file = 'token.txt' #changed 8/21/2020 changed 'tokens_LMA1' to token.txt
    ...
    for i in range(len(results)):
        ...
        first_parameter = results[i][0]
        first_length = len(first_parameter)-2
        first_parameter = first_parameter[2:first_length]
        results[i][0] = first_parameter

        second_parameter = results[i][1]
        second_length = len(second_parameter)-2
        second_parameter = second_parameter[1:second_length]
        results[i][1] = second_parameter

        third_parameter = results[i][2]
        third_length = len(third_parameter)-1
        third_parameter = third_parameter[0:third_length]
        results[i][2] = third_parameter
    ...
    #Fixed the key error: but it doesnt seem to be affecting it much as its calculation seems to be assigning 
    #Check file test.py to see fix and functionality
    ...
    for i in range(len(user_tweet_counts)):
        ...
        first_len = len(first_param)-2
        first_param = first_param[2:first_len]
